...
public class Book : MonoBehaviour
{
    ...
    public GameObject cameraPage0;
    public GameObject cameraPage1;
    public GameObject cameraPage2;
    public GameObject cameraPage3;
    public GameObject cameraPage4;
    ...
    // *** NEW: unified pointer position helper (old Input / new Input System) ***
    ...
    public Vector3 transformPoint(Vector3 pointerScreenPos)
    {
        if (canvas.renderMode == RenderMode.ScreenSpaceCamera)
        {
            Vector3 mouseWorldPos = canvas.worldCamera.ScreenToWorldPoint(
                new Vector3(pointerScreenPos.x, pointerScreenPos.y, canvas.planeDistance));
        ...
        else
        {
            ...
            Vector2 localPos = BookPanel.InverseTransformPoint(pointerScreenPos);
            return localPos;
        }
    }
    public bool IsPageDragging
    {
        get { return pageDragging; }
    }
    ...
    void Update()
    {
        ...
        // בכל פריים אחרי ש-UpdateBook הזיז את הדפים
        UpdateCameraPage();
    }
    ...
    public void UpdateBook()
    {
        ...
        f = Vector3.Lerp(f, transformPoint(pointer), Time.deltaTime * 10);
    ...
    {
        if (interactable)
            DragRightPageToPoint(transformPoint(GetPointerScreenPosition()));
    }
    ...
    {
        if (interactable)
            DragLeftPageToPoint(transformPoint(GetPointerScreenPosition()));
    }
    ...
    void UpdateCameraPage()
    {
        ...
        if (cameraPage0 != null)
        {
            ...
            if (Left != null && Right.sprite == bookPages[1] && LeftNext.sprite != bookPages[1] || Left.sprite == bookPages[0] && Right.sprite == bookPages[1] && RightNext.sprite == bookPages[0])
            {
                AttachCameraTo(Left, cameraPage0);
                AttachCameraTo(Right, cameraPage1);
        ...
        if (cameraPage1 != null && bookPages.Length > 2)
        {

            if (LeftNext.sprite == bookPages[1] && RightNext.sprite == bookPages[2])
            {
                AttachCameraTo(LeftNext, cameraPage1);
        ...
        if (
          Left != null && Right != null && LeftNext != null && RightNext != null)
        {
            if (Left.sprite == bookPages[2] &&
                Right.sprite == bookPages[3] &&
                (LeftNext.sprite == bookPages[3] || LeftNext.sprite == bookPages[1]))
            {
                if (LeftNext.sprite == bookPages[1])
                {
                    if (cameraPage1 != null) AttachCameraTo(LeftNext, cameraPage1);
                    AttachCameraTo(Right, cameraPage3);
                    AttachCameraTo(Left, cameraPage2);
                    AttachCameraTo(RightNext, cameraPage4);


                    return;
                }
            }
            if (LeftNext.sprite == bookPages[3] && RightNext.sprite == bookPages[4])
            {
                AttachCameraTo(LeftNext, cameraPage3);
                AttachCameraTo(RightNext, cameraPage4);
